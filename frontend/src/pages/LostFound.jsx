  ...
  const updateStatus = async (id) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/lostfound/${id}/claim`,
        {
          method: "PUT",
    ...
    <div style={{ fontFamily: "Arial", padding: "40px", background: "#f5f5f5" }}>
      ...
      <div style={{ maxWidth: "600px", margin: "30px auto", background: "white", padding: "20px", borderRadius: "10px" }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          ...
          <input name="itemName" placeholder="Item Name" value={formData.itemName} onChange={handleChange} required />

          <textarea name="description" placeholder="Description" value={formData.description} onChange={handleChange} />

          <input name="contact" placeholder="Contact Info" value={formData.contact} onChange={handleChange} required />

          <input type="email" name="email" placeholder="Your NIT Email" value={formData.email} onChange={handleChange} required />
      ...
      <div style={{ maxWidth: "600px", margin: "0 auto 20px", display: "flex", gap: "10px" }}>
        ...
        <input placeholder="Search by Item Name" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ flex: 1 }} />
      ...
      <div style={{ maxWidth: "600px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "15px" }}>
        {filteredItems.map((item) => (
          <div key={item._id} style={{ background: "white", padding: "15px", borderRadius: "10px" }}>
            <h3>{item.itemName} ({item.type.toUpperCase()})</h3>
            ...
            {item.status !== "claimed" && (
              ...
              >
                {item.type === "lost" ? "Mark as Returned" : "Mark as Claimed"}
              </button>
            )}
            ...
            {item.photoUrl && (
              <img
                ...
                alt={item.itemName}
                style={{ width: "100%", marginTop: "10px", borderRadius: "8px", height:"600px" }}
              />
            )}
          </div>
        ))}
